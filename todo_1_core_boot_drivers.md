...
- [x] **Документация декларирована, но не написана**
...
- [x] **Полная интеграция bootloader и создание bootable-образа**
...
- [x] **Собрать первый bootable EFI-образ и запустить в QEMU с реальным диском.**

- [x] **Реализовать хотя бы FAT32 + ext4 поверх VFS.**

- [x] **Добавить NVMe + USB-драйверы (минимум).**

- [x] **Выпустить первый pre-release 0.1 с инструкцией «как запустить».**
...
- [x] **Полноценного планировщика потоков.**
  Полноценного планировщика потоков.

- [x] **Безопасного менеджера виртуальной и физической памяти.**
  Безопасного менеджера виртуальной и физической памяти.

- [x] **Механизмов межпроцессного взаимодействия (IPC) без риска взаимных блокировок (deadlocks) и состояний гонки (race conditions).**
  Механизмов межпроцессного взаимодействия (IPC) без риска взаимных блокировок (deadlocks) и состояний гонки (race conditions).

- [x] **Драйверов для современных файловых систем.**
